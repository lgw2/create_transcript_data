...
input_gtf = list(filter(lambda line: not line.startswith('#') and line, input_gtf))
...
def process_attributes(att_line):
    return {key:value for (key,value) in map(lambda att_item: process_attribute(att_item), att_line.split(';')[:-1])}
...
def process_line(line):
    ...
    return {
        ...
        'end': int(line[4]),
        'strand': line[6],
        'transcript_id': attributes.get('transcript_id', None) if 'transcript_id' in attributes else None,
        'cov': float(attributes.get('cov', None)) if 'cov' in attributes else 1 ## One represents the unweighted case
    }

data = list(map(lambda line: process_line(line), input_gtf))

# This notebook only works with the data on the positive strand and only exons and transcripts
data = list(filter(lambda datum: datum['strand'] == '+' and (datum['feature'] == 'transcript' or datum['feature'] == 'exon'), data))
...
def add_pseudo_exons(sequence):
    ...
    #Extract exon_endpoints and sorts them according to its position and in case of ties it puts first the starting positions
    ...
    for transcript_id in sequence:
        ...
        for i, exon in enumerate(transcript['exons']):
            transcript_exons_endpoints.append({'position': exon[0], 'transcript_id': transcript_id, 'exon_index': i, 'start_point': True})
            transcript_exons_endpoints.append({'position': exon[1], 'transcript_id': transcript_id, 'exon_index': i, 'start_point': False})
        exons_endpoints.append(transcript_exons_endpoints)

    exon_endpoint_pos_list =  [item for sublist in exons_endpoints for item in sublist]
    ...
    for exon_endpoint in exon_endpoint_pos_list:
        if exon_endpoint['start_point']:
            ...
            active_exons[(exon_endpoint['transcript_id'], exon_endpoint['exon_index'])] = exon_endpoint
            ...
            del active_exons[(exon_endpoint['transcript_id'], exon_endpoint['exon_index'])]
...
def build_splicing_graphs(sequence):
    ...
    vertices = set([item for sublist in map(lambda transcript: transcript['pseudo_exons'], sequence.values()) for item in sublist])
    edges = dict()
    ...
    for (transcript_id, transcript) in sequence.items():
        ...
        for i in range(len(pseudo_exons)-1):
            ...
            if edge not in edges:
                ...
                edges[edge] = (edges[edge][0], edges[edge][1], {'cov':edges[edge][2]['cov']+cov})
...
def store_components_to_gfa(filename, components): # Write in a single file all components in gfa format
    ...
    for component in components:
        ...
        if len(list(filter(lambda transcript: len(transcript['pseudo_exons']) > 1, transcripts))) > 0: ## Only store graph with at least one transcript of length > 1
            name = ','.join(list(map(lambda transcript: transcript['transcript_id'], transcripts)))
            output_gfa.write(f'H # graph number = {graph_number} name = {name}\n')

            for edge in graph.edges:
                cov = graph.edges[edge]['cov']
                output_gfa.write(f'L\t({edge[0][0]},{edge[0][1]})\t+\t({edge[1][0]},{edge[1][1]})\t+\t{cov}\n')

            graph_number += 1
    output_gfa.close()

def store_transcripts_to_truth_file(filename, components): # Write in a single file all truth graphs in catfish's format
    ...
    for component in components:
        ...
        if len(list(filter(lambda transcript: len(transcript['pseudo_exons']) > 1, transcripts))) > 0: ## Only store graph with at least one transcript of length > 1
            name = ','.join(list(map(lambda transcript: transcript['transcript_id'], transcripts)))
            output_truth_file.write(f'# graph number = {graph_number} name = {name}\n')
